...
def maintest():
    logger = logging.getLogger("pluggabletransportadapter")
    logger.setLevel(logging.DEBUG)
    logconsole = logging.StreamHandler()
    logconsoleformatter = logging.Formatter('[%(asctime)s] %(name)-25s %(levelname)-8s %(message)s',
            '%H:%M:%S')
    #('%(threadName)s %(levelname)s: %(message)s')
    logconsole.setFormatter(logconsoleformatter)
    logconsole.setLevel(logging.DEBUG)
    ...
    s = PluggableTransportServerAdapter("./obfs4proxy", "./state", "127.0.0.1:7000",
            {
                "obfs4":{
                ...
                "obfs3":{
                    "bindaddr": "127.0.0.1:7901"
                },
                "trebuchet":{
                    "bindaddr": "127.0.0.1:1999"
                }
            })
    ...
    c = PluggableTransportClientTCPAdapter("./obfs4proxy", "./state", 
            {
                "obfs4":[
                    {
                        "listenaddr": ("127.0.0.1", 8000),
                        "remoteaddr": ("127.0.0.1", 7900),
                        "options":{
