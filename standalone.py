...
def main_cli():
    parser = argparse.ArgumentParser(
        description='Run a Tor pluggable transport as standalone server or '
                    'client.')
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        '--server', '-S', action='store_true',
        help='Run as the server end of a tunnel. '
             'Does not support --access-log yet.')
    group.add_argument(
        '--client', '-C', action='store_true',
        help='Run as the client end of a tunnel.')
    parser.add_argument(
        'configfile', type=argparse.FileType('r'),
        help='Configuration file. See the example config file for details.')
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        '--verbose', '-v', action='count',
        help='Increase verbosity level. Specify once to see logging.INFO, '
             'twice to see logging.DEBUG. Cannot be used with --quiet.')
    group.add_argument(
        '--quiet', '-q', action='store_true',
        help='Suppress all logger output to STDERR. '
             'Cannot be used with --verbose.')
    parser.add_argument(
        '--access-log', '-l',
        help='Save access log to the specified file.')
...
def get_client(config, loop):
    ...
    for s, t in config.items('transports'):
        ...
        opt = {o[8:]: v for (o, v) in config.items(s) if
               o.startswith('options-')}
        if opt:
            tr['options'] = opt
